...
if "inputs" not in ss:
    ss.inputs = dict(
        ...
        use_2x2x2=False,
        grams_glaze_per_piece_override=None,
    )
...
# ---------- Tabs ----------
h1, = st.columns(1)
h1.markdown("## Pottery Pricing App")
...
with tabs[2]:
    ...
    if up is not None:
        try:
            ...
            st.success("Settings loaded. You can switch tabs to see them applied.")
...
with tabs[3]:
    ...
    cols = st.columns(3)
    cols[0].markdown(f"Clay {money(res_full.get('clay_cost_per_piece',0))}")
    cols[1].markdown(f"Glaze {money(res_full.get('glaze_cost_per_piece',0))}")
    cols[2].markdown(f"Packaging {money(res_full.get('packaging_per_piece',0))}")
    cols = st.columns(3)
    cols[0].markdown(f"Energy {money(res_full.get('energy_per_piece',0))}")
    cols[1].markdown(f"Labor {money(res_full.get('labor_per_piece',0))}")
    cols[2].markdown(f"Overhead {money(res_full.get('overhead_per_piece',0))}")
    st.markdown(f"**Total cost** {money(res_full.get('total_cost_per_piece',0))}")

    st.markdown("#### Prices")
    if ss.inputs.get("use_2x2x2"):
        st.markdown(f"Wholesale {money(res_full.get('wholesale_price',0))}")
        st.markdown(f"Retail {money(res_full.get('retail_price',0))}")
    else:
        st.markdown(f"Wholesale from margin {money(res_full.get('wholesale_price',0))}")
        st.markdown(f"Retail from multiplier {money(res_full.get('retail_price',0))}")
    ...
    st.markdown(
        ...
        <style>\n
        @media print {\n
          header, footer, .stSidebar, [data-testid=\"stSidebar\"] { display: none !important; }\n
