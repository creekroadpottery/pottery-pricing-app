...
ss = st.session_state
if "inputs" not in ss:
    ss.inputs = dict(units_made=1, clay_price_per_bag=25.0, clay_bag_weight_lb=25.0,
                     clay_weight_per_piece_lb=1.0, clay_shrink_yield=0.9, packaging_per_piece=0.5,
                     ...
                     labor_rate=25.0, hours_per_piece=0.25, overhead_per_month=500.0, pieces_per_month=200,
                     wholesale_margin_pct=50, retail_multiplier=2.2, use_2x2x2=False)

if "glaze_table_piece" not in ss:
    ss.glaze_table_piece = pd.DataFrame([{"Material":"Frit 3134","Cost_per_kg":4.5,"Grams_per_piece":5.0},
                                         {"Material":"EPK Kaolin","Cost_per_kg":0.9,"Grams_per_piece":3.0}])

st.markdown("## Pottery Pricing App v0.3 Tabs build")
tabs = st.tabs(["Calculator","Glaze by percent","Save and load","Report","About"])

# ---------- Calculator tab ----------
with tabs[0]:
    st.markdown("### Per piece calculator")
    ...
    with left:
        ...
        ss.glaze_table_piece = st.data_editor(ss.glaze_table_piece, num_rows="dynamic",
                                              use_container_width=True, key="glaze_editor_piece")
    ...
    def calc_piece(ip, glaze_df):
        clay_cost_per_lb = ip["clay_price_per_bag"] / ip["clay_bag_weight_lb"] if ip["clay_bag_weight_lb"] else 0
        clay_cost_per_piece = (ip["clay_weight_per_piece_lb"] / ip["clay_shrink_yield"]) * clay_cost_per_lb if ip["clay_shrink_yield"] else 0
        gdf = glaze_df.copy()
        ...
        total_cost_per_piece = clay_cost_per_piece + glaze_cost_per_piece + ip["packaging_per_piece"] + energy_per_piece + labor_per_piece + overhead_per_piece
        if ip["use_2x2x2"]:
        ...
        return dict(clay_cost_per_piece=clay_cost_per_piece, glaze_cost_per_piece=glaze_cost_per_piece,
                    packaging_per_piece=ip["packaging_per_piece"], energy_per_piece=energy_per_piece,
                    labor_per_piece=labor_per_piece, overhead_per_piece=overhead_per_piece,
                    total_cost_per_piece=total_cost_per_piece, wholesale_price=wholesale_price,
                    retail_price=retail_price, distributor_price=distributor_price, glaze_detail=gdf)
...
with tabs[1]:
    st.markdown("### Glaze cost from percent recipe")
    st.caption("Enter a materials catalog with cost per kilo. Enter a recipe in percents and a batch size in grams. The app finds the price, converts to grams, then totals the batch and per piece cost.")

    st.markdown("Materials catalog")
    ss.materials_catalog = st.data_editor(
        df_safe(ss.materials_catalog, ["Material","Cost_per_kg"]),
        num_rows="dynamic",
        use_container_width=True,
        key="materials_catalog_editor_v2",   # unique key
    )

    st.markdown("Recipe in percent")
    ss.recipe_percent = st.data_editor(
        df_safe(ss.recipe_percent, ["Material","Percent"]),
        num_rows="dynamic",
        use_container_width=True,
        key="recipe_percent_editor_v2",      # unique key
    )

    colA, colB = st.columns(2)
    with colA:
        ss.recipe_batch_g = st.number_input(
            "Batch size in grams",
            min_value=1.0,
            value=float(ss.recipe_batch_g),
            step=10.0,
            key="batch_size_in_grams_v2",    # unique key
        )
    with colB:
        ss.recipe_grams_per_piece = st.number_input(
            "Grams used per piece",
            min_value=0.0,
            value=float(ss.recipe_grams_per_piece),
            step=0.5,
            key="grams_per_piece_v2",        # unique key
        )
    ...
    st.dataframe(out_df.assign(Cost=out_df["Cost"].map(money)), use_container_width=True)
    c1, c2, c3 = st.columns(3)
...
with tabs[3]:
    ...
    st.caption("Use your browser print command to save as PDF. This view hides inputs and shows results only.")
    ...
    st.markdown(
        '<style>@media print {header, footer, .stSidebar, [data-testid="stSidebar"] { display: none !important; } section.main .block-container { padding: 0 !important; }}</style>',
